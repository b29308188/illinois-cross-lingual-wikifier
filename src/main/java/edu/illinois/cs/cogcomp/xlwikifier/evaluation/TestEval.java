...
//                docs = reader.readDocs("/shared/corpora/corporaWeb/tac/2017/LDC2017E25_TAC_KBP_2017_Evaluation_Source_Corpus/data/eng/nw/", "en");
        ...
        we.loadDB("es",true);
        BufferedWriter bw = new BufferedWriter(new FileWriter("nom_test.txt"));
        for ( ELMention m: golds ) {
            float[] v = we.getWordVector(m.getSurface(), "en");
            if(v == null || v.length < 1) continue;
            bw.write(LabelMap.get(m.getType()) + " ");
            for (int i = 0; i < v.length; i++) {
                bw.write(String.valueOf(i + 1) + ":" + String.valueOf(v[i]) + " ");
            }
            bw.write("\n");
        }
        bw.close();
